...
def load(input_path) -> pd.DataFrame:
...
def norm_experiment(df, outdir):
    df['dataset_metric'] = df['problem'] + '_' + df['metric']

    # Global CD analysis
    _ = stat_test(df, treatment='norm_formal', datasets='dataset_metric', acc='acc', alpha=0.1, super_treatment='category', outname=os.path.join(outdir, '4b-normalization_all.pdf'), 
                height=2.4, 
    ...
    # Overall ranking
    overall_rank = df.groupby(['dataset_metric', 'norm']).acc.mean().unstack().rank(ascending=False, axis=1).mean(axis=0).sort_values().index.tolist()

    # Category specific
    cat_ranks = {}
    for cat in df.category.unique():
        cat_ranks[cat] = df[df.category == cat].groupby(['dataset_metric', 'norm']).acc.mean().unstack().rank(ascending=False, axis=1).mean(axis=0).sort_values().index.tolist()
...
def elastic(df, unsup, sbd_ref, outdir):
    view = df[
...
def kernel(df, unsup, sbd_ref, outdir):
    view = df[
...
def feature(df, l2ref, outdir):
    view = df[
        ((df.category == 'Feature-based') | (df.metric == 'l2')) &
        (df.norm == 'nonorm')
    ]

    stats = type_analysis(view[view.metric != 'l2'], l2ref, ref_name=('l2', 'nonorm'), alpha=0.05)
...
def embedding(df, l2_ref, outdir):
    view = df[
...
def dependency(df, outdir):
    channel_view = df[
        (df.norm == 'nonorm') &
        ~(df.metric.str.endswith('-100'))
    ]
...
def main(input_path, output_dir):
    ...
    with open(os.path.join(OUTDIR, 'tables.txt'), 'w') as f:
        ...
        norm_experiment(unsup, OUTDIR)
        lockstep(df, L2_REF, OUTDIR)
        sliding(df, LOR_REF, OUTDIR)
        elastic(df, unsup, SBD_REF, OUTDIR)
        kernel(df, unsup, SBD_REF, OUTDIR)
        feature(df, L2_REF, OUTDIR)
        embedding(df, L2_REF, OUTDIR)
        ensemble(df, OUTDIR)
        dependency(unsup, OUTDIR)
        runtime(unsup, OUTDIR)
