from src.libraries.ts2vec.encode import encode
from src.distance_measures.lock_step.lp import euclidean_all
import numpy as np

def ts2vec_d_all(X_test, X_train, adaptive_scaling, n_jobs=None, **kwargs):
    train_repr, test_repr = encode(X_train=X_train, X_test=X_test, **kwargs)
...
def ts2vec_i_all(X_test, X_train, adaptive_scaling, n_jobs=None, **kwargs):

    rep_length = kwargs.get('repr_dims', 320)
    max_channels = 100
    ...
    for i in range(c):
        ...
        X_rep_c, Y_rep_c = encode(X_train=Xc, X_test=Yc, **kwargs)
