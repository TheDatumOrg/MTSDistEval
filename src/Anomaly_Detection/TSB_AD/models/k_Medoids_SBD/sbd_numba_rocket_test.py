...
"""
@njit
def SBD_Local_all_rocket(X,Y,adaptive_scaling=False):
    ...
    X_ffts = np.ascontiguousarray(X_ffts)
    Y_ffts = np.ascontiguousarray(Y_ffts)
...
@njit
def SBD_Global_all_rocket(X,Y,adaptive_scaling=False):
