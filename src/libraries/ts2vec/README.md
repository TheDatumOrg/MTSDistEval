...
# (Both X_train and X_test have a shape of n_instances x n_timestamps x n_features)
...
test_repr = model.encode(X_test)  # n_instances x n_timestamps x output_dims

# Compute instance-level representations for test set
test_repr = model.encode(X_test, encoding_window='full_series')  # n_instances x output_dims
