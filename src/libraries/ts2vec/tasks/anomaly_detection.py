...
def eval_ad_result(test_pred_list, y_test_list, test_timestamps_list, delay):
    labels = []
    pred = []
    for test_pred, y_test, test_timestamps in zip(test_pred_list, y_test_list, test_timestamps_list):
        assert test_pred.shape == y_test.shape == test_timestamps.shape
        y_test = reconstruct_label(test_timestamps, y_test)
        test_pred = reconstruct_label(test_timestamps, test_pred)
        test_pred = get_range_proba(test_pred, y_test, delay)
...
def eval_anomaly_detection(model, all_X_train, all_y_train, all_train_timestamps, all_X_test, all_y_test, all_test_timestamps, delay):
    t = time.time()
    ...
    all_train_repr_wom = {}
    all_test_repr_wom = {}
    for k in all_X_train:
        X_train = all_X_train[k]
        X_test = all_X_test[k]

        full_repr = model.encode(
            np.concatenate([X_train, X_test]).reshape(1, -1, 1),
            mask='mask_last',
            causal=True,
            sliding_length=1,
            sliding_padding=200,
        ...
        all_train_repr[k] = full_repr[:len(X_train)]
        all_test_repr[k] = full_repr[len(X_train):]
        ...
        all_train_repr_wom[k] = full_repr_wom[:len(X_train)]
        all_test_repr_wom[k] = full_repr_wom[len(X_train):]
...
def eval_anomaly_detection_coldstart(model, all_X_train, all_y_train, all_train_timestamps, all_X_test, all_y_test, all_test_timestamps, delay):
    t = time.time()
    ...
    all_repr_wom = {}
    for k in all_X_train:
        all_data[k] = np.concatenate([all_X_train[k], all_X_test[k]])
    ...
    for k in all_data:
        data = all_data[k]
        labels = np.concatenate([all_y_train[k], all_y_test[k]])
