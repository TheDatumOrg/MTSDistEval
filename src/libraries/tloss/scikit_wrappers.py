...
import src.libraries.tloss.utils as utils
import src.libraries.tloss.losses as losses
import src.libraries.tloss.networks as networks
...
class TimeSeriesEncoderClassifier(sklearn.base.BaseEstimator,
    ...
    def fit_encoder(self, X, y=None, save_memory=False, verbose=False):
        ...
        while i < self.nb_steps:
            for j, batch in enumerate(train_generator):
                if self.cuda:
                ...
                loss.backward()
                self.optimizer.step()
                print(f"Epoch: {epochs + 1}, step {j}, loss: {loss}")
                i += 1
                if i >= self.nb_steps:
                    break
            epochs += 1
            # Early stopping strategy
            if self.early_stopping is not None and y is not None:
                ...
                try:
                    ...
                    print("Error in cross-validation: ", e)
                    score = numpy.mean(sklearn.model_selection.cross_val_score(
                        self.classifier, features, y=y, cv=2, n_jobs=2
                    ))


                if score == 1:
                    print('Early stopping: perfect score')
                    break
                ...
                print(F"Early stopping strikes: {str(count)}, Score: {str(score)}")
...
class CausalCNNEncoderClassifier(TimeSeriesEncoderClassifier):
    ...
    def set_params(self, params: Hyperparameters):
        self.__init__(
            compared_length = params.compared_length, 
            nb_random_samples = params.nb_random_samples, 
            negative_penalty = params.negative_penalty, 
